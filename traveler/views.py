	...
	if request.method=="POST":
		if request.POST['action']=="Forgot Password":
			...
			try:
				...
				order=Transaction.objects.filter(made_by=user)
...
def seller_hotel_detail(request,pk):
	...
	if hotels.hotel_name=="The Paradise Inn":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	elif hotels.hotel_name=="Hotel deLuxe":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	elif hotels.hotel_name=="Four Seasons":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	elif hotels.hotel_name=="Hi-Way Inn":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	elif hotels.hotel_name=="Candlewood Suites":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	elif hotels.hotel_name=="Cute Mountains":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	elif hotels.hotel_name=="Hotel Bliss":
		hotelgallery=HotelGallery.objects.filter(hotels=hotels)
		print(hotelgallery)
	else:
		pass
...
def hotel_detail(request,pk):
	...
	return render(request,'hotel_details.html',{'hotels':hotels,'flag':flag,'flag1':flag1,'hotelgallery':hotelgallery})
...
def user_hotel_detail(request,pk):
	...
	return render(request,'user_hotel_details.html',{'hotels':hotels,'flag':flag,'flag1':flag1,'hotelgallery':hotelgallery})
...
def user_flight_detail(request,pk):
	...
	flights=Flights.objects.get(pk=pk)
	try:
		FlightCart.objects.get(user=User_Signup.objects.get(email=request.session['email']),flights=flights)
		flag1=True
	except Exception as e:
		pass
...
def user_cab_detail(request,pk):
	...
	cabs=Cabs.objects.get(pk=pk)
	try:
		CabCart.objects.get(user=User_Signup.objects.get(email=request.session['email']),cabs=cabs)
		flag1=True
	except Exception as e:
		pass
...
def my_booking_order(request):
	...
	return render(request,'mybookingorder.html',{'flightcart':flightcart,'hotelcart':hotelcart,'cabcart':cabcart,'flag':flag, 'net_price':net_price})
